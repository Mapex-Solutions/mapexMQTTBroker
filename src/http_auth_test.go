...
// recordingDoer captures every HTTP request the AuthClient sends so
// tests can assert headers, URL, and call count without standing up a
// real server.
...
func TestLookupEntry_EmptyUUIDReturnsNotFound(t *testing.T) {
	...
	if _, err := a.LookupEntry(context.Background(), ""); !errors.Is(err, ErrAuthEntryNotFound) {
...
// TestLookupEntry_AgainstRealServer exercises the actual http.Client
// (not the recordingDoer) against an in-process httptest.Server. This
// validates timeout + transport tuning + header propagation end-to-end.
...
func TestCompareLocal_MatchesHash(t *testing.T) {
	// Generate a fresh hash at cost 4 (cheap for tests; bcrypt salts
	// are random per invocation, so a hardcoded fixture would be
	// non-portable across rebuilds).
