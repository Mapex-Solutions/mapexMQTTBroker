...
//
// All branching, parsing, and NATS interaction lives in the broker
// package (parent directory) where it is unit-testable. This file's
// only job is the cgo glue — boilerplate exported C entry points and
// trampolines that delegate to the package's PluginRuntime methods.
//
// Layering:
//
//	cmd/plugin/main.go     ← cgo trampolines, broker thread; thin
//	../plugin.go           ← PluginRuntime methods; Go-pure
//	../acl.go              ← ACL logic; Go-pure
//	../nats_publisher.go   ← async pool; Go-pure
//
// The build is gated by the `cgo_mosquitto_plugin` build tag so a
// default `go build ./...` and `go test ./...` run without requiring
// libmosquitto-dev installed; only the Dockerfile multi-stage builder
// compiles with this tag.
package main
...
import (
	...
	broker "github.com/Mapex-Solutions/mapexMQTTBroket/src"
...
// parse reuses ResolveUsername without divergence.
...
// runtime is the process-wide PluginRuntime, set in
...
var runtime atomic.Pointer[broker.PluginRuntime]
...
func mosquitto_plugin_init(
	...
	_ = userdata
	log := broker.NewLogger(nil, broker.LogInfo)
	log.Info("plugin_init: starting")

	opts := readOpts(rawOpts, optCount)
	cfg, err := broker.LoadConfig(opts)
	if err != nil {
		log.Error("plugin_init: config load failed err=%v", err)
		return C.MOSQ_ERR_UNKNOWN
	}

	nc, err := broker.ConnectNATS(cfg.NatsURL, log)
	if err != nil {
		log.Error("plugin_init: nats connect failed err=%v", err)
		return C.MOSQ_ERR_UNKNOWN
	}

	natsConn.Store(nc)

	rt, err := broker.BuildRuntime(broker.RuntimeOptions{
		Config:    cfg,
		Publisher: nc,
		NatsConn:  nc,
		Log:       log,
	...
	if err != nil {
		log.Error("plugin_init: runtime build failed err=%v", err)
		nc.Close()
		return C.MOSQ_ERR_UNKNOWN
	}
	runtime.Store(rt)
	...
	if rc := registerCallbacks(identifier); rc != C.MOSQ_ERR_SUCCESS {
		log.Error("plugin_init: callback registration failed rc=%d", int(rc))
		_ = rt.Async.Drain(broker.ShutdownTimeout)
		nc.Close()
		runtime.Store(nil)
		natsConn.Store(nil)
		return rc
	}
	log.Info("plugin_init: ready (4 callbacks registered)")
...
func mosquitto_plugin_cleanup(userdata unsafe.Pointer, opts *C.struct_mosquitto_opt, optCount C.int) C.int {
	...
	rt.Log.Info("plugin_cleanup: draining")
...
func goOnBasicAuth(event C.int, eventData unsafe.Pointer, userdata unsafe.Pointer) C.int {
	...
	ctx, cancel := contextWithTimeout(rt.Config.AuthTimeout)
	defer cancel()
	res := rt.Authenticate(ctx, username, password, clientID, certSerial)

	switch res {
	case broker.AuthAllow:
		// On success, also emit the presence.connect advisory so
		// the healthmonitor sees the online edge — Mosquitto 2.0.x
		// has no MOSQ_EVT_CONNECT, so this is the canonical online
		// signal. The trusted orgId was captured into the session
		// map by Authenticate; LookupSession returns false for
		// service accounts that bypassed the device auth path.
		if info, ok := rt.LookupSession(clientID); ok {
			rt.PublishConnect(info.OrgID, info.AssetUUID, clientID, sourceIP, time.Now().UTC())
		}
		return C.MOSQ_ERR_SUCCESS
	case broker.AuthDeny:
...
func goOnAclCheck(event C.int, eventData unsafe.Pointer, userdata unsafe.Pointer) C.int {
	...
	if broker.CheckACL(username, topic, access) {
		return C.MOSQ_ERR_SUCCESS
	}
...
func goOnDisconnect(event C.int, eventData unsafe.Pointer, userdata unsafe.Pointer) C.int {
	...
	info, ok := rt.LookupSession(clientID)
	if !ok {
		// No prior Authenticate / unknown clientID — service account
		// or stale callback. Nothing to publish, nothing to clean up.
		return C.MOSQ_ERR_SUCCESS
	}
	rt.PublishDisconnect(info.OrgID, info.AssetUUID, clientID, sourceIP, int(ed.reason), time.Now().UTC())
	rt.ForgetSession(clientID)
	return C.MOSQ_ERR_SUCCESS
}
...
func goOnMessage(event C.int, eventData unsafe.Pointer, userdata unsafe.Pointer) C.int {
	...
	rt.PublishIngress(info.OrgID, info.AssetUUID, clientID, topic, payload,
